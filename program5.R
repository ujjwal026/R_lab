...
generate_report <- function() {
  ...
  report$Attendance[is.na(report$Attendance)] <- 100  # Filling NA attendance with 100
  cat("Performance Report:\n")
  print(report)
}

# Function to validate input values (0-100 and not NA)
validate_input <- function(scores) {
  if (all(!is.na(scores) & scores >= 0 & scores <= 100)) {
    return(TRUE)
  } else {
    return(FALSE)
  }
...
while (TRUE) {
  ...
  if (choice == 1) {
    ...
    scores <- c(math_score, science_score, history_score, attendance)
    if (validate_input(scores)) {
      # Add student to the data
      add_student(name, math_score, science_score, history_score, attendance)
    } else {
      cat("Invalid input. Please enter valid numeric values between 0 and 100.\n")
    }
  } else if (choice == 2) {
    generate_report()
